...
START_DATE = {
    ...
    # 'daily': dt.datetime(1998, 6, 1),
    'daily': dt.datetime(2017, 1, 1),
    'index_weight': dt.datetime(2005, 1, 1)
}


class TushareData2(DataSource):
    ...
    def update_calendar(self) -> None:
        """ 更新上交所交易日历 """
        table_name = 'mg_trade_day_a'
        df = self._pro.trade_cal(is_open=1)
        if self.db_interface.exist_table(table_name=table_name):
            self.db_interface.purge_table(table_name)
        self.db_interface.insert_df(df, table_name)
        logging.getLogger(__name__).info('上交所交易日历下载完成.')

    def update_hk_calendar(self) -> None:
        """ 更新港交所交易日历 """
        table_name = 'mg_trade_day_h'
        df = self._pro.hk_tradecal(is_open=1)
        if (self.db_interface.exist_table(table_name=table_name)):
            self.db_interface.purge_table(table_name)
        self.db_interface.insert_df(df, table_name)

        logging.getLogger(__name__).info('港交所交易日历下载完成.')
    ...
    def update_stock_list_date(self) -> None:
        ...
        storage = []
        list_status = ['L', 'D', 'P']
        table_name = 'mg_security_code';
        if self.db_interface.exist_table(table_name=table_name):
            self.db_interface.purge_table(table_name=table_name)
        for status in list_status:
            df = self._pro.stock_basic(exchange='', list_status=status)
            self.db_interface.insert_df(df=df, table_name=table_name)
            storage.append(df)
    ...
    def update_convertible_bond_list_date(self) -> None:
        ...
        table_name = 'mg_bond_info_transferable';
        logging.getLogger(__name__).debug(f'开始下载{data_category}.')
        output = self._pro.cb_basic(fields=list(desc.keys()))
        if self.db_interface.exist_table(table_name=table_name):
            self.db_interface.purge_table(table_name=table_name)
        self.db_interface.insert_df(output, table_name=table_name)
        # list date
    ...
    def update_future_list_date(self) -> None:
        ...
        for exchange in constants.FUTURE_EXCHANGES:
            df = self._pro.fut_basic(exchange=exchange, fields=list(desc.keys()) + ['per_unit'])
            df['future_type'] = exchange
            self.db_interface.insert_df(df=df, table_name=table_name)
            storage.append(df)
    ...
    def update_option_list_date(self) -> None:
        ...
        desc = self._factor_param[data_category]['输出参数']
        table_name = 'mg_future_option_info'
        if self.db_interface.exist_table(table_name=table_name):
            self.db_interface.purge_table(table_name=table_name)
        logging.getLogger(__name__).debug(f'开始下载{data_category}.')
        storage = []
        for exchange in constants.FUTURE_EXCHANGES + constants.STOCK_EXCHANGES:
            df = self._pro.opt_basic(exchange=exchange, fields=list(desc.keys()))
            self.db_interface.insert_df(df=df, table_name=table_name)
        #     storage.append(df)
        # output = pd.concat(df)
    ...
    def update_fund_list_date(self) -> None:
        ...
        desc = self._factor_param[data_category]['输出参数']
        table_name = 'mg_fund_info'
        if self.db_interface.exist_table(table_name=table_name):
            self.db_interface.purge_table(table_name=table_name)
        logging.getLogger(__name__).debug(f'开始下载{data_category}.')
        storage = []
        for market, status in itertools.product(['E', 'O'], ['D', 'I', 'L']):
            df = self._pro.fund_basic(market=market, status=status, fields=list(desc.keys()))
            self.db_interface.insert_df(df=df, table_name=table_name)
            storage.append(df)
        # output = pd.concat(storage)
    ...
    def get_shibor(self, start_date: DateUtils.DateType = None, end_date: DateUtils.DateType = None) -> pd.DataFrame:
        """ Shibor利率数据 """
        table_name = 'mg_shibor_rate'
        desc = self._factor_param['Shibor利率数据']['输出参数']
    ...
    def get_company_info(self) -> pd.DataFrame:
        ...
        table_name = 'mg_company_info'
        column_desc = self._factor_param[data_category]['输出参数']
        fields = list(column_desc.keys())
        logging.getLogger(__name__).debug(f'开始下载{data_category}.')
        storage = []
        for exchange in constants.STOCK_EXCHANGES:
            df = self._pro.stock_company(exchange=exchange, fields=fields)
            # self.db_interface.insert_df(df=df, table_name=table_name)
            storage.append(df)
        df = pd.concat(storage)
        self.saveDataToMysql(df, table_name=table_name)
    ...
    def get_ipo_info(self, start_date: DateUtils.DateType = None) -> pd.DataFrame:
        ...
        table_name = 'mg_security_code_new'
        column_desc = self._factor_param[data_category]['输出参数']
        logging.getLogger(__name__).debug(f'开始下载{data_category}.')
        df = self._pro.new_share(start_date=start_date)
        self.saveDataToMysql(df, table_name)
    ...
    def saveDataToMysql(self, df, table_name):
        if self.db_interface.exist_table(table_name=table_name):
            self.db_interface.purge_table(table_name=table_name)
        self.db_interface.insert_df(df, table_name=table_name)
        return
    ...
    def get_daily_hq(self, trade_date: DateUtils.DateType = None,
        ...
        :return: None
        """
        table_name = "mg_stock_daily2"
        table_name_factor = "mg_stock_daily_factor2"
        if (not trade_date) & (not start_date):
            existTable = self.db_interface.exist_table(table_name)
            lastDay = START_DATE['daily'] if not existTable else self.db_interface.get_column_max(table_name,'trade_date')
            start_date = lastDay if lastDay is not None else START_DATE['daily']
        ...
        with tqdm(dates) as pbar:
            for date in dates:
                ...
                if not self.db_interface.exist_table(table_name):
                    ...
                    self.db_interface.update_df(df=df,table_name=table_name)

                # adj_factor data
                df = self._pro.adj_factor(trade_date=current_date_str)
                if  not self.db_interface.exist_table(table_name_factor):
                    self.db_interface.insert_df(df=df,table_name=table_name_factor)
                else:
                    self.db_interface.update_df(df=df,table_name=table_name_factor)
                table_name_basic = "mg_stock_daily_basic2"
                # indicator data
                df = self._pro.daily_basic(trade_date=current_date_str)
                if  not self.db_interface.exist_table(table_name_basic):
                    self.db_interface.insert_df(df=df, table_name=table_name_basic)
                else:
                    self.db_interface.update_df(df=df, table_name=table_name_basic)
    ...
    def get_hs_holding(self, date: DateUtils.DateType):
        ...
        table_name = 'mg_hk_hold'

        df = self._pro.hk_hold(trade_date=date)
        self.db_interface.delete_id_records()
        self.db_interface.insert_df(df=df, table_name=table_name)
    ...
    def update_hs_holding(self) -> None:
        """ 沪深港股通持股明细 """
        data_category = '沪深港股通持股明细'
        table_name = 'mg_ggt_hold_info'
        # self.db_interface.__init__(self.db_interface);
        self.db_interface.delete_id_records()
    ...
    def _format_list_date(df: pd.DataFrame, extend_delist_date: bool = False) -> pd.Series:
        ...
        output = TushareData2._standardize_df(output, {'ts_code': 'ID', 'list_date': 'DateTime'})
    ...
    def format_ticker(tickers: Union[Sequence[str], str]) -> Union[Sequence[str], str]:
        if isinstance(tickers, str):
            return TushareData2._format_ticker(tickers)
        else:
            return [TushareData2._format_ticker(it) for it in tickers]
